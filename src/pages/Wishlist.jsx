...
const Wishlist  = () =>{
    ...
    <Container fluid>
        ...
        <Col lg={2}>
            ...
            <Col lg={2}>
                <Container fluid>
                    <Row className="delete-btn" onClick={()=>{deleteList(wishlist)}}>
                        <Col lg={1}>
        ...
        <Row className="products-row mt-5">
            <Container fluid>
                ...
                <Row >
            <Col> <span  className="saved-items-title" >{(wishlists_loading)?(<></>):(<>{wishlist && wishlist.data[0].attributes.wishlist_name}</>)}</span> </Col>
        </Row>
                <Row >
                {(wishlists_loading) ? (
        <Loader />
      ...
      {(wishlist && wishlist.data.map((data)=>{
        return  data.attributes.products.data.map((product)=>{
              return(<Col lg={3} className="p-2">
              <Container fluid>
                  <Row>
                      <Col >
                      <img src={product.attributes.product_inventories.data[0].attributes.images.data[0].attributes.formats.medium.url} height="100%" width="100%" />
                      </Col>
                  </Row>
                  <Row>
                      <Col>
                  ...
                  <Row>
                      <Col>
                      <Container fluid>
                          <Row><Col><span className="rent-price">AED {product.attributes.twelve_month_price/12}/mo</span></Col>
                          <Col><span className="buy-price">AED {product.attributes.price} to buy</span></Col>
                          </Row></Container></Col>
                  </Row>
              </Container>
              </Col>)
